...
export interface PostRunCoachingOpts {
  targetPaceSec?: number;
  targetRpe?: string;
}

export function buildPostRunCoaching(detail: RunDetail, opts: PostRunCoachingOpts = {}): string {
  ...
  if (splits.length >= 3) {
    ...
    const avg = (arr: typeof splits) =>
      arr.reduce((a, x) => a + ((x.moving_time || x.elapsed_time || 0) / Math.max((x.distance || 1000) / 1000, 0.1)), 0) / arr.length;
...
function adjustTargetByReadiness(targetPaceSec: number, readinessScore: number): number {
...
export interface LiveCoachingOpts {
  targetPaceSec?: number;
  maxHr?: number;
  hrSustainedSec?: number;
...
export interface CoachingDecision {
  ...
  nextCheckSec: number;
  adjustedTargetPaceSec: number;
}

export function buildLiveCoachingDecision(metrics: LiveMetrics, opts: LiveCoachingOpts = {}): CoachingDecision {
  ...
  const diff = paceSec - targetPaceSec;
  let paceCue: string;
  let action: CoachingDecision['action'] = 'maintain';
  let severity: CoachingDecision['severity'] = 'info';
...
export function buildLiveCoaching(metrics: LiveMetrics, opts: LiveCoachingOpts = {}): string {
